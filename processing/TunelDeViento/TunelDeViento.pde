 ...
 * Editado por Brayan Angarita
 * Simulación - UNAULA
 * Octubre de 2019
 * Este programa permite conocer el comportamiento del viento sobre un objeto. En este caso, una camioneta
 */
...
import processing.opengl.PGraphics2D;
  ...
  private class MyFluidData implements DwFluid2D.FluidData{
    ...
    // esto se llama durante el paso de actualización de simulación de fluidos.
    public void update(DwFluid2D fluid) {
      ...
      // usa el texto como entrada para la densidad
    ...
    // sombreador personalizado, para agregar velocidad desde una textura (PGraphics2D) al fluido.
...
//      pg_tex_handle[0] = pg.getTexture().glName
  ...
  // forma animada de metamorfosis, utilizada como obstáculo dinámico
...
//___________________________________________________________________________________________________________________   
    // Interfaz para Agregar DATOS (Fluidos)
    cb_fluid_data = new MyFluidData();
    fluid.addCallback_FluiData(cb_fluid_data);

    // processing font
    fonto = createFont("../data/SourceCodePro-Regular.ttf", 48);
    ...
    // Textura Del Obstaculo Principal
    ...
    // Algunos Obstaculos Iniciales
    for(int i = 0; i < 50; i++){
      float px = 200;
      float py = 200;
   
      // mantener libre el área central izquierda
      if(    py > height/2 - 120 
          && py < height/2 + 120
          && px < width/2 + 100
          ){
      ...
      pg_obstacles_drawing.fill(204,43,54);//Color del vehiculo
      pg_obstacles_drawing.rect(500,360, 250, 70,20); //Estructura del auto
      pg_obstacles_drawing.rect(455,320, 40, 50,20);//Parte superior del auto
      
      pg_obstacles_drawing.rectMode(CENTER);
      pg_obstacles_drawing.stroke(12);
      pg_obstacles_drawing.fill(255);
      pg_obstacles_drawing.rect(580,400, 55, 55,180,180,180,180);//Llanta trasera
      pg_obstacles_drawing.rect(430,400, 55, 55,180,180,180,180);//Llanta delantera
    }
    
    pg_obstacles_drawing.translate(200, height/2+50);
    pg_obstacles_drawing.rotate(0.3f);
    pg_obstacles_drawing.fill(225,255,227);
  ...
  //__________________________________________________________________________________________________________________  
    ...
    // imagen / búfer que se utilizará como entrada de densidad
